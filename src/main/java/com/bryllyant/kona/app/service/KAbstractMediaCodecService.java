...
public abstract class KAbstractMediaCodecService implements KMediaCodecService {
	private static Logger logger = LoggerFactory.getLogger(KAbstractMediaCodecService.class);
	

	
	/**
	 * Return full path to ffmpeg binary.
	 * 
	 * @return
	 */
	protected abstract String ffmpeg();
	

	
	protected abstract String ffprobe();
    ...
    @Override
   	...
   	public byte[] convertMP4(String format, byte[] video, BigDecimal startPosition, BigDecimal duration) throws IOException {
  		...
  		try {
  			...
  			logger.debug("convertMP4: KScript: START");
  			KScript proc = new KScript(ffmpeg(), args);
              proc.run();
  			logger.debug("convertMP4: KScript: END");
  		} catch (KScriptException e) {
  			throw new IOException(e);
  		}
