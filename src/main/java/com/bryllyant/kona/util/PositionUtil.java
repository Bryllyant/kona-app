...
public class PositionUtil {
	private static Logger logger = LoggerFactory.getLogger(PositionUtil.class);
    ...
    */
    ...
    public static double getDistanceTraveled(List<Position> positionList)  {
    ...
    public static Position getFurthestPosition(Position start, List<Position> positionList)  {
        // Calculate the total distance traveled
        Position max = null;
    ...
    public static double getAverageAltitude(List<Position> positionList)  {
        double altitude = 0.0;
    ...
    public static double getAverageGrade(List<Position> positionList)  {
        double grade = 0.0;
    ...
    public static Long getCumulativeTime(List<Position> positionList) {
        int count = positionList.size();
        
        if (count < 2) return 0L;

        Position first = positionList.get(0);
        Position last = positionList.get(count-1);
        
        Long startTime = first.getPositionDate().getTime();
        Long endTime = last.getPositionDate().getTime();
    ...
    public static double getAverageHorizontalAccuracy(List<Position> positionList) {
        double accuracy = 0.0;
        
        if (positionList.size() > 0) {
            for (int i=0; i<positionList.size(); i++) {
                accuracy += positionList.get(i).getHorizontalAccuracy();
            }
